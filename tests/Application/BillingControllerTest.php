...
{
    private LemonSqueezyClient&MockObject $lsClient;
    private LicenseManager&MockObject     $licenseManager;
    private BillingController             $controller;

    protected function setUp(): void
    {
        $this->lsClient       = $this->createMock(LemonSqueezyClient::class);
    ...
    public function testCheckoutReturns404WhenUserNotFound(): void
    {
        $this->licenseManager->method('findUser')->willReturn(null);

        $request  = $this->authedRequest(['variant_id' => '123'], userId: 99);
        $response = $this->controller->checkout($request);

        $this->assertSame(404, $response->getStatusCode());
    }

    public function testCheckoutReturns422WhenVariantIdMissing(): void
    {
        $this->licenseManager->method('findUser')->willReturn($this->fakeUser());

        $request  = $this->authedRequest([], userId: 1);
        $response = $this->controller->checkout($request);

        $this->assertSame(422, $response->getStatusCode());
        $body = json_decode((string) $response->getBody(), true);
        $this->assertStringContainsString('variant_id', $body['error']);
    }
    ...
    public function testCheckoutReturns422WhenVariantIdIsBlank(): void
    {
        ...
        $request  = $this->authedRequest(['variant_id' => '   '], userId: 1);
        $response = $this->controller->checkout($request);
    ...
    public function testCheckoutReturnsCheckoutUrl(): void
    {
        ...
        $request  = $this->authedRequest(['variant_id' => 'variant-42'], userId: 1);
        $response = $this->controller->checkout($request);
    ...
    public function testCheckoutReturns502WhenLsClientThrows(): void
    {
        ...
        $request  = $this->authedRequest(['variant_id' => 'v1'], userId: 1);
        $response = $this->controller->checkout($request);
